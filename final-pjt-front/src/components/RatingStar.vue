  ...
  methods: {
    createRating(n) {
      ...
      axios({
        ...
        headers: {
          Authorization: `Token ${authStore.token}` // 스토어의 토큰 사용
        },
        data: { score: n }
      })
      .then((res) => {
      ...
      rating(n) {
        ...
        for (let i = 1; i <= n; i++) {
          this.score.push(i);
          this.notScore.splice(this.notScore.indexOf(i), 1);
        }
        this.$emit('ratingSelected', n);
      },
    ...
    axios({
      method: 'get',
      url: `${API_URL}/accounts/rating/${movie_id}/${authStore.username}`, // 스토어의 username 사용
    })
    .then(res => {
