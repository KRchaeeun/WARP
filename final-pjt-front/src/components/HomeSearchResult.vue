  ...
  <div>
    <div id="movie-search">
      <div v-if="searchInputData" class="movie-search--info">
        "<strong>{{searchInputData}}</strong>"의 검색결과 <strong>{{ searchResultData.length }}</strong>건
      </div>
...
export default {
  ...
  methods: {
    ...
    const apiKey = 'Your API Key'
      axios({
        method: 'get',
        url: `https://api.themoviedb.org/3/search/movie?query=${this.searchInputData}&api_key=${apiKey}&language=ko-KR&page=1`,
      })
        .then((res) => {
          // 인기도 순 출력
